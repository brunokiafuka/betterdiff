...
ipcMain.handle(
  "github:compareRefs",
  async (_event, repoFullName: string, base: string, head: string) => {
    if (!octokit) {
      throw new Error("Not authenticated");
    }
...
ipcMain.handle(
  ...
  async (_event, repoFullName: string, ref: string, filePath: string) => {
    if (!octokit) {
      throw new Error("Not authenticated");
    }
...
ipcMain.handle(
  "github:getRepoTree",
  async (_event, repoFullName: string, ref: string) => {
    if (!octokit) {
      throw new Error("Not authenticated");
    }
...
ipcMain.handle(
  "github:getFileHistory",
  async (_event, repoFullName: string, filePath: string, ref: string) => {
    if (!octokit) {
      throw new Error("Not authenticated");
    }
...
ipcMain.handle(
  "github:getCommit",
  async (_event, repoFullName: string, sha: string) => {
    if (!octokit) {
...
ipcMain.handle(
  "github:getBlame",
  async (_event, repo: string, ref: string, path: string) => {
  ...
  async (
    ...
    try {
      ...
      if (
        ...
        errorMessage.includes("overwritten")
      ) {
        throw new Error(
          "LOCAL_CHANGES: You have uncommitted changes that would be overwritten. Please commit, stash, or discard them first.",
        );
      }
  ...
  try {
    ...
    try {
      ...
      remoteBranches = remoteBranchesOutput
        ...
        .filter(
          (b): b is { name: string; sha: string; type: "branch" } => b !== null,
        );
    } catch (e) {
      // No remotes configured
      console.error("No remote branches found or error:", e);
    }
    ...
    try {
      ...
      tags = tagsOutput
        ...
        .filter(
          (t): t is { name: string; sha: string; type: "tag" } => t !== null,
        );
    } catch (e) {
      // No tags
    }
    ...
    const uniqueRefs = new Map<
      string,
      { name: string; sha: string; type: "branch" | "tag" }
    >();
...
ipcMain.handle(
  "local:compareRefs",
  async (_event, repoPath: string, base: string, head: string) => {
    try {
      // Get diff stats
      const diffOutput = execGitCommand(
        repoPath,
        `diff --stat ${base}...${head}`,
      );

      // Get list of changed files
      const filesOutput = execGitCommand(
        repoPath,
      ...
      const files = filesOutput
        ...
        .map((line) => {
          ...
          const statLine =
            execGitCommand(
              repoPath,
              `diff --numstat ${base}...${head} -- "${filePath}"`,
            ).split("\n")[0] || "";
          ...
          try {
            if (fileStatus !== "added") {
              oldSha = execGitCommand(
                repoPath,
                `rev-parse ${base}:${filePath}`,
              ).split("\n")[0];
            }
            if (fileStatus !== "deleted") {
              newSha = execGitCommand(
                repoPath,
                `rev-parse ${head}:${filePath}`,
              ).split("\n")[0];
            }
          } catch (e) {
      ...
      const commitsOutput = execGitCommand(
        repoPath,
        `log --format="%H|%an|%ae|%ad|%s" --date=iso ${base}..${head}`,
      );
...
ipcMain.handle(
  ...
  async (_event, repoPath: string, ref: string, filePath: string) => {
    try {
...
ipcMain.handle(
  ...
  async (_event, repoPath: string, ref: string) => {
    try {
      const treeOutput = execGitCommand(
        repoPath,
...
ipcMain.handle(
  "local:getFileHistory",
  async (_event, repoPath: string, filePath: string, ref: string) => {
    try {
      const commitsOutput = execGitCommand(
        repoPath,
        `log --format="%H|%an|%ae|%ad|%s" --date=iso ${ref} -- "${filePath}"`,
      );
  ...
  async (_event, repoPath: string, sha: string) => {
    try {
      const commitInfo = execGitCommand(
        repoPath,
        `show --format="%H|%an|%ae|%ad|%cn|%ce|%cd|%s" --stat --date=iso ${sha}`,
      );
      ...
      if (statLine) {
        const statMatch = statLine.match(
          /(\d+)\s+file.*?(\d+)\s+insertion.*?(\d+)\s+deletion/,
        );
        if (statMatch) {
      ...
      const files = filesOutput
        ...
        .map((line) => {
          ...
          return {
            ...
            changes:
              (statMatch ? parseInt(statMatch[1]) : 0) +
              (statMatch ? parseInt(statMatch[2]) : 0),
          };
        })
        .filter((f): f is NonNullable<typeof f> => f !== null);
...
ipcMain.handle(
  ...
  async (_event, repoPath: string, ref: string, filePath: string) => {
    try {
      const blameOutput = execGitCommand(
        repoPath,
        `blame -l ${ref} -- "${filePath}"`,
      );
      ...
      lines.forEach((line, index) => {
        const match = line.match(
          /^([a-f0-9]+)\s+\((.+?)\s+(\d{4}-\d{2}-\d{2})\s+(\d+:\d+:\d+)\s+([+-]\d+)\s+(\d+)\)\s+(.+)$/,
        );
        if (match) {
  ...
  async (
    ...
    try {
      ...
      for (const stats of Array.from(fileStats.values())) {
        if (stats.changeCount > maxChangeCount)
          maxChangeCount = stats.changeCount;
      ...
      for (const [path, stats] of Array.from(fileStats.entries())) {
        ...
        const recencyScore =
          stats.recencyWeights.length > 0
            ? stats.recencyWeights.reduce((a: number, b: number) => a + b, 0) /
              stats.recencyWeights.length
            : 0;

        // Normalize metrics (0-1 range)
        const normalizedChangeCount =
          maxChangeCount > 0 ? stats.changeCount / maxChangeCount : 0;
...
ipcMain.handle(
  "local:hotspot:analyze",
  async (_event, repoPath: string, ref: string, timeWindow: number = 30) => {
    try {
      // Validate repo path
      ...
      try {
        ...
        commitsOutput = execGitCommand(
          repoPath,
          `log --format="%H|%an|%ae|%ad|%s" --date=iso --since="${daysAgo}" ${ref}`,
        );
      } catch (err: any) {
      ...
      const commitLines = commitsOutput
        .split("\n")
        .filter((line) => line.trim());
      ...
      for (const commit of commitsToAnalyze) {
        ...
        try {
          ...
          const filesOutput = execGitCommand(
            repoPath,
            `diff-tree --no-commit-id --name-status -r ${commit.sha}`,
          );
          const fileLines = filesOutput
            .split("\n")
            .filter((line) => line.trim());
          ...
          for (const fileLine of fileLines) {
            ...
            try {
              const statLine = execGitCommand(
                repoPath,
                `diff --numstat ${commit.sha}^..${commit.sha} -- "${filePath}"`,
              );
              if (statLine) {
                const statMatch = statLine.match(/^(\d+)\s+(\d+)/);
                if (statMatch) {
